...
5. If your own voice shows up in the transcript a second time (often garbled, in another language, or as another speaker) and you use a voice changer or virtual audio device such as Voicemod, VB-Cable or OBS monitoring, open Settings → Preferences → System Audio and set **Playback Device** to **Default Playback Device Only**. OpenWhispr then records only the device Windows plays sound through (the setting shows its name), so make sure your meeting app plays there too. The debug log's `Meeting system audio source` line shows which source and capture path each recording used
