...
if (!swiftTarget) {
  console.error(`[notch-probe] Unsupported architecture: ${targetArch}`);
  process.exit(1);
}

const projectRoot = path.resolve(__dirname, "..");
const swiftSource = path.join(projectRoot, "resources", "macos-notch-probe.swift");
const outputDir = path.join(projectRoot, "resources", "bin");
const outputBinary = path.join(outputDir, "macos-notch-probe");
const hashFile = path.join(outputDir, `.macos-notch-probe.${targetArch}.hash`);
...
function log(message) {
  console.log(`[notch-probe] ${message}`);
}
...
if (!fs.existsSync(swiftSource)) {
  console.error(`[notch-probe] Swift source not found at ${swiftSource}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  console.error("[notch-probe] Failed to compile macOS notch probe binary.");
...
try {
  ...
  console.warn(`[notch-probe] Unable to set executable permissions: ${error.message}`);
}

// Verify the compiled binary matches the target architecture
if (!verifyBinaryArch(outputBinary, targetArch)) {
  console.error(
    `[notch-probe] FATAL: Compiled binary architecture does not match target (${targetArch}). ` +
...
log(`Successfully built macOS notch probe binary (${targetArch}).`);
