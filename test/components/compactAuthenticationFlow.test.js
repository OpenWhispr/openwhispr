...
test("only an address restored from a saved session opens verification as resumed", async (t) => {
  ...
  harness.values = {};
  harness.drafts.length = 0;
  const signUp = renderFlow({ resumeState: { pendingVerificationEmail: null } });
  assert.equal(signUp.type.name, "AuthenticationStep");
  signUp.props.onNeedsVerification("person@example.com");

  const verifying = renderFlow({ resumeState: { pendingVerificationEmail: null } });
  assert.equal(verifying.type.name, "EmailVerificationStep");
  assert.equal(verifying.props.resumed, false);
  // The address has to reach the session, or the next launch cannot restore it.
  assert.deepEqual(harness.drafts, [{ pendingVerificationEmail: "person@example.com" }]);
});
