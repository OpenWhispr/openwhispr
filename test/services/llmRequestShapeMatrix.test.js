...
const MATRIX = [
  ...
  ["local llama.cpp: chat_template_kwargs is the only thinking switch, no Ollama think flag (#783)", "local", "qwen3.5-4b-gguf", null, CLEANUP_NO_THINK,
    { max_tokens: MAX_TOKENS, temperature: 0, chat_template_kwargs: { enable_thinking: false } }],
  ["local registry qwen3 carries supportsThinking, so it is suppressed", "local", "qwen3-4b-q4_k_m", null, CLEANUP_NO_THINK,
    { max_tokens: MAX_TOKENS, temperature: 0, chat_template_kwargs: { enable_thinking: false } }],
  ["local registry gemma-4 thinks by default under llama-server (template <|think|> switch), so it is suppressed", "local", "gemma-4-e4b-it-qat-q4_0", null, CLEANUP_NO_THINK,
    { max_tokens: MAX_TOKENS, temperature: 0, chat_template_kwargs: { enable_thinking: false } }],
  ["local fail-safe: a registry model with no thinking mode still gets the off switch (llama-server ignores it, and decides think-by-default itself)", "local", "gemma-3-4b-it-q4_k_m", null, CLEANUP_NO_THINK,
    { max_tokens: MAX_TOKENS, temperature: 0, chat_template_kwargs: { enable_thinking: false } }],
  ["lan keeps the supportsThinking exemption — the fail-safe is local-only", "lan", "gemma-3-4b-it-q4_k_m", null, CLEANUP_NO_THINK,
    { max_tokens: MAX_TOKENS, temperature: 0 }],
  ["local gpt-oss cleanup: family low pin rides in chat_template_kwargs, never top-level (llama-server b9763)", "local", "gpt-oss-20b-mxfp4", null, CLEANUP,
    { max_tokens: MAX_TOKENS, temperature: 0, chat_template_kwargs: { reasoning_effort: "low" } }],
  ["local gpt-oss cleanup with suppression: pin and floor share the kwargs object", "local", "gpt-oss-20b-mxfp4", null, CLEANUP_NO_THINK,
    { max_tokens: MAX_TOKENS, temperature: 0, chat_template_kwargs: { enable_thinking: false, reasoning_effort: "low" } }],
  ["local gpt-oss agent with suppression: floor still rides in kwargs, no cleanup pin", "local", "gpt-oss-20b-mxfp4", null, AGENT_NO_THINK,
    { max_tokens: MAX_TOKENS, temperature: 0.3, chat_template_kwargs: { enable_thinking: false, reasoning_effort: "low" } }],
  ["local gpt-oss selection edit pins low effort in kwargs like cleanup", "local", "gpt-oss-20b-mxfp4", null, SELECTION_EDIT,
    { max_tokens: MAX_TOKENS, temperature: 0.3, chat_template_kwargs: { reasoning_effort: "low" } }],
