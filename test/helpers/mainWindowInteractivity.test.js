...
for (const platform of ["linux", "win32"]) {
  // Electron ignores the `forward` option outside macOS, so a pill returned to
  // click-through never sees another mouseenter: on Linux the cancel X showed
  // up on the first hover and never again (#2136), the same limitation that
  // stranded the meeting card's buttons in #1456.
  test(`${platform} keeps the dictation pill hit-testable across hovers`, () => {
    assert.deepEqual(hoverCycle(platform), [INTERACTIVE, INTERACTIVE, INTERACTIVE]);
  });
}
