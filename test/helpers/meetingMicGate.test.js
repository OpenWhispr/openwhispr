// test/helpers/meetingMicGate.test.js
...
const cases = [
  ...
  ["streaming mic silence zeroes", "streaming", "mic", 0.001, 0.04, false, { action: "zero", reason: "silence" }],
  ["streaming mic silence needs BOTH rms and peak below", "streaming", "mic", 0.001, 0.06, false, { action: "send", reason: null }],
  ["streaming mic quiet + system speaking zeroes (bleed floor)", "streaming", "mic", 0.01, 0.06, true, { action: "zero", reason: "bleed_floor" }],
  ["streaming mic quiet + system silent sends", "streaming", "mic", 0.01, 0.06, false, { action: "send", reason: null }],
  ["streaming mic at the rms ceiling is not quiet (strict <)", "streaming", "mic", 0.018, 0.06, true, { action: "send", reason: null }],
  ["streaming mic at the peak ceiling is not quiet (strict <)", "streaming", "mic", 0.01, 0.07, true, { action: "send", reason: null }],
  ["streaming loud mic sends", "streaming", "mic", 0.1, 0.5, true, { action: "send", reason: null }],
  ["streaming system source is never gated", "streaming", "system", 0.0, 0.0, true, { action: "send", reason: null }],
  ["local silence skips any source", "local", "system", 0.001, 0.04, false, { action: "skip", reason: "silence" }],
  ["local mic quiet + system speaking skips (system-dominant)", "local", "mic", 0.01, 0.06, true, { action: "skip", reason: "system_dominant" }],
  ["local system quiet + system speaking is not gated", "local", "system", 0.01, 0.06, true, { action: "send", reason: null }],
...
test("resolveMicChunkAction sends empty chunks unchanged in both modes", () => {
  for (const mode of ["streaming", "local"]) {
    assert.deepEqual(
      gate.resolveMicChunkAction({ mode, source: "mic", rms: 0, peak: 0, sampleCount: 0, isSystemSpeaking: () => true }),
...
test("resolveMicChunkAction only consults isSystemSpeaking when the chunk is quiet (preserves the local short-circuit)", () => {
  let calls = 0;
  gate.resolveMicChunkAction({ mode: "local", source: "mic", rms: 0.5, peak: 0.9, sampleCount: 800, isSystemSpeaking: () => (calls += 1, true) });
  assert.equal(calls, 0);
  gate.resolveMicChunkAction({ mode: "local", source: "mic", rms: 0.01, peak: 0.06, sampleCount: 800, isSystemSpeaking: () => (calls += 1, false) });
