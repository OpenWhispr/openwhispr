...
test("deepseek gets its native thinking switch and never reasoning_effort or chat_template_kwargs", async () => {
  const { suppressThinking } = await load();

  const body = {};
  suppressThinking(body, "deepseek", "deepseek-v4-flash");

  assert.deepEqual(body, { thinking: { type: "disabled" } });
  assert.ok(!("reasoning_effort" in body), "DeepSeek rejects reasoning_effort none with a 400");
  assert.ok(!("chat_template_kwargs" in body), "DeepSeek has no chat_template_kwargs switch");
});
...
test("detectEndpointDialect maps the deepseek api base to max_tokens and temperature", async () => {
  const { detectEndpointDialect } = await load();

  assert.deepEqual(detectEndpointDialect("https://api.deepseek.com/v1"), {
    key: "deepseek",
    tokenParam: "max_tokens",
    supportsTemperature: true,
  });
