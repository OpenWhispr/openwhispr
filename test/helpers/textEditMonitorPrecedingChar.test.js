...
test("getPrecedingText resolves to unknown for missing pid", async () => {
  const m = new TextEditMonitor();
  for (const pid of [null, undefined, 0]) {
    assert.deepEqual(await m.getPrecedingText(pid), { state: "unknown" });
  }
});

test("getPrecedingText returns unknown when the AX read fails or hangs", async () => {
  const m = new TextEditMonitor();
  ...
  const result = await m.getPrecedingText(99999999, 1500);
