...
test("mouse button hotkeys 3 through 32 are macOS-only and cannot combine with keyboard keys", async () => {
  const { validateHotkey } = await load();

  assert.equal(validateHotkey("MouseButton3", "darwin").valid, true);
  assert.equal(validateHotkey("MouseButton4", "darwin").valid, true);
  assert.equal(validateHotkey("MouseButton32", "darwin").valid, true);
  ...
  assert.equal(validateHotkey("MouseButton2", "darwin").valid, false);
  assert.equal(validateHotkey("MouseButton33", "darwin").valid, false);
});
