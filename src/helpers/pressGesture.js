// Double-press ("hands-free") gesture detection for Hold mode, shared by every
// push-to-talk path. Pure logic: callers pass timestamps and perform the side
// effects themselves, so the state machine stays unit-testable without timers.
//
// Hold mode only, on purpose: in Tap mode a single press already starts an
// unlimited recording, so a double press there would add nothing but a second
// way to do the same thing while taking the ordinary "stop" meaning away from
// a fast second press.
//
