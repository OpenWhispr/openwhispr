...
class WhisperVulkanManager {
  ...
  async download(progressCallback) {
    ...
    try {
      ...
      debugLogger.info("Vulkan whisper binary download starting", {
      ...
      zipPath = path.join(tempDir, `vulkan-whisper-download-${Date.now()}.zip`);
      extractDir = path.join(tempDir, `temp-vulkan-whisper-extract-${Date.now()}`);
      ...
      debugLogger.info("Vulkan whisper binary download complete", {
        version: releaseInfo.version,
  ...
  async delete() {
    ...
    debugLogger.info("Vulkan whisper binary deleted", { deletedCount, freedBytes });
