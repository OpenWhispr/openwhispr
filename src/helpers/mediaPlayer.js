...
class MediaPlayer {
  ...
  _pauseMacOS() {
    ...
    // Try direct AppleScript control for known players first.
    // This keeps pause/resume state-aware even when MediaRemote is unavailable
    // or when a player does not expose an active MediaRemote session.
    const apps = [
      {
        name: "Spotify",
        pauseCmd: 'tell application "Spotify" to pause',
        checkCmd: 'tell application "Spotify" to player state as string',
        playing: "playing",
      },
      {
        name: "Music",
        pauseCmd: 'tell application "Music" to pause',
        checkCmd: 'tell application "Music" to player state as string',
        playing: "playing",
      },
    ];
    ...
    let mediaRemoteOutput = "";
    if (binary) {
      const result = spawnSync(binary, ["--pause"], {
        stdio: "pipe",
        timeout: 3000,
      });
      mediaRemoteOutput = (result.stdout?.toString() || "").trim();
      if (result.status === 0) {
    ...
    // Best-effort fallback for non-scriptable desktop players such as Brain.fm.
    ...
    // exit 1 = nothing was playing, don't fallback to toggle blindly
    if (mediaRemoteOutput === "NOT_PLAYING") return false;
  ...
  _resumeMacOS() {
    ...
    for (const appName of apps) {
      ...
      if (appName.startsWith("__ui__:")) {
        const uiAppName = appName.slice("__ui__:".length);
        if (this._resumeMacOSViaUiScripting(uiAppName)) {
          resumed = true;
        }
        continue;
      }
...
`.trim();
  ...
  _pauseMacOSViaUiScripting() {
    ...
    const labels = ["Pause", "Pause Session", "Pause Music", "Pause Playback", "Play/Pause"];
    const pausedApps = [];
  ...
  _resumeMacOSViaUiScripting(appName) {
    const labels = ["Play", "Resume", "Resume Session", "Play Session", "Play/Pause"];
