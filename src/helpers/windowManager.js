...
class WindowManager {
  ...
  startNativePushToTalk(key, inputKind = DICTATION_INPUT_KIND.DICTATION) {
    ...
    const safetyTimeoutId = setTimeout(() => {
      ...
      this.handleNativePushKeyUp(undefined, { reason: "timeout" });
    }, MAX_PUSH_DURATION_MS);
  ...
  // "release" is the user letting go; every other reason ends a push whose
  // trigger keys are still physically down, which the renderer must know
  // before it pastes.
  handleNativePushKeyUp(key, { reason = "release" } = {}) {
  ...
  resetNativePushState() {
    ...
    this.handleNativePushKeyUp(undefined, { reason: "reset" });
  }
