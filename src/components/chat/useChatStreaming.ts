...
import type { ToolExecutionContext, ToolRegistry } from "../../services/tools/ToolRegistry";
    ...
    async (userText: string, allMessages: Message[], options?: SendToAIOptions) => {
      ...
      const discardAssistant = () => {
        ...
        setMessages((previousMessages) =>
          previousMessages.filter((message) => message.id !== assistantId)
        );
      };
      ...
      const authorization = captureRuntimeAuthorizationLease(
        "reasoning",
        handleAuthorizationInvalidation
      );
      const toolExecutionContext: ToolExecutionContext = {
        assertAuthorized: () => authorization.assertCurrent(),
      };
      const authorizationCancelled = () => {
        if (!authorization.isCurrent()) handleAuthorizationInvalidation();
        return authorizationInvalidated;
      };
      ...
      try {
        ...
        const announceResponse = () => {
          if (responseAnnounced || authorizationCancelled()) return;
        ...
        if (authorizationCancelled() || cancelled() || !mountedRef.current) return;
        ...
        setMessages((prev) => [
          ...prev,
          { id: createdAssistantId, role: "assistant", content: "", isStreaming: true },
        ]);
          ...
          if (isCloudAgent) {
            const executeToolCall = registry
              ? async (name: string, argsJson: string) => {
                  ...
                  const result = await tool.execute(args, toolExecutionContext);
                  authorization.assertCurrent();
            ...
            const aiTools = registry?.toAISDKFormat(toolExecutionContext);
