...
export default function DictionaryView() {
  ...
  const dictionarySelection = buildDictionaryPrompt(dictionaryEntries, {
    provider: useLocalWhisper ? "local-whisper" : cloudTranscriptionProvider || "openai",
    agentName,
  });
  const activeMode = useLocalWhisper
    ? t("dictionary.modeLocal", "local")
    : t("dictionary.modeCloud", "cloud");
  const selectionSummary =
    dictionarySelection.droppedEntries > 0
      ? t("dictionary.summaryWithDrop", {
          stored: dictionaryEntries.length,
          mode: activeMode,
          selected: dictionarySelection.selectedEntries.length,
          dropped: dictionarySelection.droppedEntries,
        })
  ...
  const handleImportGlossary = useCallback(async () => {
    setIsImportingGlossary(true);
    try {
      const result = await importOtterGlossaryDictionary();
      if (result?.success) {
        setImportText("");
        setImportOpen(false);
      }
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                >
                  {isImportingGlossary
                    ? t("dictionary.importGlossaryBusy", "Importing glossary...")
                    : t("dictionary.importGlossary", "Import Claude glossary")}
                </Button>
                <input
                  ref={fileInputRef}
                  type="file"
      ...
      {isEmpty ? (
        ...
        <>
          ...
          <div className="px-5 pb-2">
            <p className="text-xs text-foreground/35 leading-relaxed">{selectionSummary}</p>
          </div>
