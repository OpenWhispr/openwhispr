...
## Full-app verification checkpoint — September 25

The follow-up built and preserved a current normally signed full simulator app and refreshed the runtime/PR/environment mapping. Full iPhone and iPad staging anonymous Account checks passed typing versus explicit on-screen Go/native same-app Paste, consent refusal, exact full-link retention, repeat entry and supported keyboard layouts. iPhone additionally passed an untrusted-link refusal and empty recovery. Cloud led directly to existing consent and continued after refusal, but the shared paywall itself was not displayed. The separate iPad fictional paywall probe passed Monthly/Annual switching, layout and X dismissal.

An observed iPad landscape field obstruction was fixed at `f582525e` by opting Account's existing scroll view into automatic keyboard insets. Three affected suites / 41 tests and local formatting/lint/typecheck passed; two independent reviews approved, and GitHub Mobile CI / CodeQL CI passed. Local Expo Doctor was 19/20 because the shell lacked CocoaPods; the normal signed Xcode simulator build succeeded. No new implementation-mirroring test was added for native geometry; actual before/after simulator observations establish the bounded repair.

Cross-app clipboard permission, physical devices, floating/split/external keyboards, the full identity/session/offline/already-attributed/unavailable matrix and real catalog callbacks remain open. Apple sign-in was completed, but its authenticated browser window could not be read through the current attachment; Josh asked to stop that loop. Existing AASA excludes the development bundle, and simulators have no StoreKit config. Current storefront/catalog readback and a reachable owned test runtime whose bundle owns the subscriptions are the next prerequisites. All real creator saving/allocation, Apple financial acceptance and production activation remain held. See the handover's new checkpoint and native evidence for rig locations and the later Apple run card.

## Next fresh session: full-app paywall and real catalog

Josh chose this as the next session's priority after the native checkpoint and asked to retain all other open items. First refresh the owned branch/#2334 and perform a read-only identity/catalog/runtime check against the completed staging setup: actual Apple Monthly/Annual products, current prices/currency/storefront, subscription-owning bundle, Superwall/RevenueCat/API mapping and a reachable owned test device with a compatible signed binary. Apple sign-in was completed; do not repeat the failed browser-attachment/sign-in loop. The current dev/probe identities and absent simulator StoreKit config are not real catalog proof.

Then establish an isolated delivery path for the existing approved paywall in the full app without broadening probe/live campaigns. Observe the screen from the intended Cloud/ordinary Account path, actual Monthly/Annual product data and correct `products.secondary.identifier` at both callback boundaries. Preserve all native guards and service refusal. If a creator-offer happy path or redeem callback requires a real creator claim or offer allocation, carry that dependency into the separately authorized Apple session; do not perform a financial or attribution mutation to make this verification pass. Fictional/intercepted callback data must remain labeled as such.

The new linked handover contains the complete open-items table: physical iPhone/iPad and keyboard modes, cross-app paste permission, registered/anonymous/subscribed/already-attributed and session/offline/unavailable matrix, actual universal-link delivery, current Apple stock/eligibility, later redemption/purchase/signup recovery/restore and signed server commission/refund acceptance, production onboarding association, desktop single-writer canary/installer acceptance, monitoring/payout ownership and coordinated activation. Reuse passed native Account tests; repair only observed defects on the owned branch with affected checks and independent review. No production change, creator saving/allocation, redemption, purchase/restore or financial mutation is authorized by this kickoff.
...
The separate fixture established native input/selected-plan keys, returned price/renewal rendering and cached repeat reset. Native follow-up established seed/token display/action keys and explicit clearing on cached repeat. The real product identifier remains absent from the isolated bundle. The full-app checkpoint above supersedes the earlier native Account Paste/Go and simulator layout gap; physical devices, cross-app clipboard permission and the complete state matrix remain pending. The separate probe passed missing-token retry, malformed-price refusal, reachable regular recovery, pending input locking and X/late completion isolation. A compatible new binary is required for the native creator input; an OTA to an old binary is insufficient. Any local runtime proof must retain a distinct development identity and avoid personal-app replacement.
