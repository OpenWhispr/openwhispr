...
- **Slow-waking microphones no longer hard-fail every dictation.** Bluetooth headsets, wireless dongles, and freshly plugged USB mics can take one to two seconds to deliver their first audio after the device opens, and closing and reopening the device restarts that wake-up. Since 1.9.0 every dictation verified the mic within 600ms and reopened it on failure, so these devices could never come up: every press ended in a "no audio" error loop. The first open now waits out the device's wake (up to 2.5s, with zero added latency for mics that respond instantly — the wait ends the moment audio flows), and the recovery hops to a fresh open and the system default keep their fast checks. The debug log also now records when the fallback mic was silent too and when a recording start fails, which previously left no trace.
