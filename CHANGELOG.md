...
- **AssemblyAI is a bring-your-own-key transcription provider.** AssemblyAI's Universal models (`universal-3-5-pro` and `universal-2`) join OpenAI, Groq, xAI, Mistral, and Gemini for dictation and uploads. Transcription runs through AssemblyAI's pre-recorded API and returns paragraph-formatted text, and the key is stored encrypted like every other provider key. Batch only, so it stays out of the streaming-only meeting picker.
