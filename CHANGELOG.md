...
- **The wake word is gone.** Starting a dictation with your assistant's name ("Hey Jarvis, rewrite this") no longer sends it to the Voice Assistant; the words are cleaned up and typed like any other dictation. Routing on a fuzzy name match was the main reason a dictation came back as an answer instead of the words you said, and it kept growing special cases for greetings, languages, and snippet triggers. The Voice Assistant hotkey, which states the intent explicitly, is now the only way to give a spoken command. It has no default key, so if yours is unset, Settings → AI Models → Voice Assistant points you to Hotkeys to add one. (#2182)
