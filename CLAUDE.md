...
- **connectors/**: Agent connectors
  ...
  - `connector_actions` (SQLite) is the durable receipt: destination labels, states and links only, never content. Rows belong to the signed-in account (Recent actions lists only that account's; account deletion removes them). Interrupted rows are reconciled on launch (committing → unknown), expired pending actions are recorded as `expired`, and a direct run that throws or returns a malformed result is `unknown` (it may have happened), never `failed`
  ...
  - `emailConnector.js` opens Gmail/Outlook/mailto compose windows (`emailCompose.js`, shared ESM); a body that won't fit the compose URL (2,000 chars; 6,000 for Gmail on macOS/Linux, since Windows caps opened URLs near 2,081) goes to the clipboard after the OS accepts the link (on Linux a `mailto` with no `xdg-mime` handler is refused first), then the subject too if it's still too long; a recipient list that alone doesn't fit is refused. `email_draft` opens at most 3 drafts per turn, and only one of them may use the clipboard (`ToolExecutionContext.claimTurnSlot`; a draft that didn't open gives its slots back). Automatic picks Outlook work vs Outlook.com from the Microsoft account's tenant id (`tid`, consumer tenant `9188040d-…`), falling back to the email domain for accounts connected before it was stored
  - `contactSearch.js` (`find_contact`) searches the meetings nearest to now (`calendar_events` only spans the sync window; cancelled and declined meetings don't count) plus the `contacts` table, most recently synced first, behind the same org policy check as the actions. The user's calendar accounts, `self` attendees (except on a colleague's shared Google calendar, where `self` marks its owner), an Apple organizer who is the current user, `resource` attendees (rooms, flagged by all three calendar managers) and `*.calendar.google.com` addresses are excluded; the calendar managers also keep the user and rooms out of `contacts` and purge those addresses when they see them again. Each `contacts` row records its `source` (`google:<account>`, `microsoft:<account>`, `apple`, `manual`); disconnecting an account deletes its rows, and the lookup reads only manual rows and connected accounts' rows (rows from before the column existed are left out; note-participant autocomplete still reads every row)
...
7. Connector tools (email_draft, find_contact; paid, policy-gated) run through src/helpers/connectors. Only typed chat (`ChatView`) and this panel opt in (`allowConnectors`); note chat, container chat and the onboarding demo don't get them. An approval card, any `email_draft` (its compose window takes focus) and any `find_contact` (a lookup is almost always followed by a question for the user) hold the turn in the panel (`onHoldDelivery`) instead of pasting at the caret. A held answer is still auto-copied, except after `email_draft`, which may have put overflow text on the clipboard (`onHoldDelivery({ preserveClipboard: true })`)
