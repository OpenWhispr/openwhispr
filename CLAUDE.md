...
- **autoStartPolicy.js**: Electron-free launch-at-login and relaunch decisions
  ...
  - `getRelaunchOptions()` shapes the `relaunch-app` IPC that follows `cleanup-app` (Reset app data, Delete account's device erase), which closes the database and stops local services for good. The relaunch drops `--hidden` (so it never comes up in the tray) and any cold-start deep link (so startup doesn't handle it twice); under `npm run dev` it only quits. On AppImage, Electron's relaunch helper runs from the FUSE mount and dies with it, so the handler instead spawns a detached `/bin/sh` that waits for the app to exit and then starts `$APPIMAGE`
