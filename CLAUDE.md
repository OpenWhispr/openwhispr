...
- `submitAfterPasteEnabled`: Press Enter after a dictation is pasted (see Submit After Paste)
...
### 19. Submit After Paste

Settings → General → Clipboard → "Press Enter after pasting" (`submitAfterPasteEnabled`, off by default, needs auto-paste) presses Enter once a dictation is pasted, so chat messages, prompts and terminal commands go out hands-free. Terminals are not excluded.

- **Who decides**: `useAudioRecording.js` passes `submitKey: "enter"` only on a plain dictation paste of an intact result. Never on selection edits or Voice Assistant replies, and never when cleanup failed, the transcript is partial (`result.warning`) or a translation fell back to the source text (`result.translationFallback`): Enter can't be taken back, so those stay in the field for review. The `paste-text` handler accepts only `"enter"` and skips smart spacing's trailing space (decided before the paste, so a route that ends up not pressing Enter leaves no space either). When Enter was actually pressed it skips auto-learn and stops an earlier monitor, since the field is cleared
- **Same call as the paste**: the native helpers take `--submit enter --submit-delay <ms>` (`SUBMIT_DELAY_MS` = 250 in `clipboard.js`; helper timeouts grow by `SUBMIT_TIMEOUT_MS`, the delay plus room to press Enter). After the paste they wait, check, press Return and print `SUBMIT_OK` / `SUBMIT_SKIPPED <reason>` / `SUBMIT_FAILED`, always exiting 0 once the paste went out: a failing exit or a timeout sends the caller to a fallback that pastes again. A helper too old for the flag pastes once and prints no `SUBMIT_*` line, so the submit is reported as `helper-unsupported` instead of pasting twice
  - macOS: Return with empty event flags, so a held hotkey modifier can't leak in; skipped when the frontmost app changed. The helper waits on the run loop, not `usleep`: a command-line process only sees `NSWorkspace.frontmostApplication` change while its run loop runs
  - Windows: skipped when the foreground window changed or a modifier is held (a second release/restore bracket could leave a key stuck); advertises `submit-v1`
  - Linux: Return on the portal (same session, so the single-use restore token is not spent twice; the token and the `SUBMIT_*` line share stdout) and XTest routes, skipped when #2113's modifier state reads held (an unreadable state presses Enter, as the paste gate does); XTest also checks the active window. Routes that paste through a system tool (wtype, ydotool, xdotool) press Enter as a second command from the same tool after the same delay and `_awaitModifierRelease(0)`: one check, no waiting, because focus can move while a key is held (Alt+Tab). wtype and ydotool can't see focus; xdotool skips Enter when `getactivewindow` no longer matches the window it pasted into
- **Pastes but never submits**: the macOS osascript and Windows nircmd/PowerShell fallbacks, the throwaway uinput device on any compositor (a compositor can pick up the hotplugged device late, dropping the paste yet taking the later Enter, #956, which would send the previous draft), Hyprland `sendshortcut` (its latched modifier would turn Enter into Ctrl+Enter or Shift+Enter), and a Wayland clipboard write where wl-copy is installed but failed (`_writeClipboardWayland()` returns false; the paste could insert the previous clipboard). `clipboard.js` logs the submit outcome of every completed paste as "Submit after paste" with its reason
- **Tests**: `test/helpers/clipboardRestore.test.js` (routes, outcomes, token split, timeout budget), `test/helpers/ipcPasteOutcome.test.js`, `test/helpers/useAudioRecordingForceStop.test.js`, `test/helpers/audioManagerCleanupFallback.test.js`
