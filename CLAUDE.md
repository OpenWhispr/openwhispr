...
- User names their assistant in Settings → AI Models → Voice Assistant (`src/utils/agentName.ts`; localStorage `agentName`, default "OpenWhispr")
- The name is the assistant's persona in the `dictationAgent` prompt (`{{agentName}}`) and is auto-synced into the custom dictionary so STT spells it correctly
- There is no wake word: saying the name during dictation is ordinary content. The assistant is invoked only by the Voice Assistant hotkey (section 17)
