...
7. Connector tools (`email_draft`, `find_contact`; paid, policy-gated) run through `src/helpers/connectors`. Only typed chat (`ChatView`) and this panel opt in (`allowConnectors`); note chat, container chat and the onboarding demo don't get them. An approval card, any `email_draft` (its compose window takes focus) and any `find_contact` (a lookup is almost always followed by a question for the user) hold the turn in the panel (`onHoldDelivery`) instead of pasting at the caret. A held answer is still auto-copied, except after an `email_draft` that reserved the clipboard for overflow text (`onHoldDelivery({ preserveClipboard: true })`, sticky for the turn) and on a caret turn whose delivery restores the clipboard (Keep Transcription in Clipboard off)
