...
- **systemAudioSource.js**: The Windows system audio source setting (#1546; users see it as Settings → Preferences → System Audio → Playback Device), dependency-free so main `require()`s it and the renderer imports it. `all-devices` (default) keeps the native process-loopback helper; `default-device` sends capture to Chromium's renderer loopback of the Windows default output and never probes or starts the helper during a meeting (cloud connect and prepare ask `getMeetingSystemAudioPlanMode()`, which answers Windows without the helper). The renderer reads the setting once per meeting start and passes it to both `checkSystemAudioAccess` and `meetingTranscriptionStart`, so the access check and main's plan agree; `normalizeSystemAudioSource()` maps anything unknown to `all-devices`. Downstream stages key on mode (`loopback` either way), so AEC, echo-leak muting, live speaker identification and diarization are unchanged. UI: `src/components/settings/SystemAudioSourceSettings.tsx`, which renders its own section directly below Microphone on the `general` page on Windows, and nothing elsewhere. Tests: `test/helpers/windowsSystemAudioSourcePlan.test.js`, `test/stores/meetingRecordingStoreSystemAudioSource.test.js`
